...
### 3.3. Premium Tasarım, Tema ve Estetik Mimarisi (Design System)
Tüm sistem arayüzleri, son kuşağın en yüksek estetik standartlarına (Premium SaaS UX) göre insan eliyle özenle kodlanmışçasına tasarlanmıştır. "Sıradan" bir yazılım / panel görünümünden kesinlikle kaçınılmıştır:
* **Tipografi ve Görsel Hiyerarşi:** Google Fonts'un modern yüzleri olan `Inter` ve `Outfit` kullanılarak kusursuz ve keskin bir okuma deneyimi sunulur.
* **Renk Paleti (Curated HSL):** Jenerik basit renkler (düz kırmızı, yeşil, mavi) kullanılmaz. Özel HSL tabanlı, derinliği olan harmonik ve çok zengin dark/light mode renk paletleri tercih edilir.
* **Glassmorphism ve Akıcı Dinamikler:** Özellikle Web Dashboard (Planlayıcı) ve Öğretmen modüllerinde cam efekti (Glassmorphism), derin blurlar ve soft-gradient alan zeminleri kullanılır.
* **Mikro-Animasyonlar:** Sistemin "canlı ve duyarlı" hissettirmesi için hover (üzerine gelme) efektleri ve Framer Motion destekli yumuşak (smooth) geçişler ile (Örn: bildirim rozeti sıçramaları, şoför uyarı parlamaları) üst düzey kullanıcı etkileşimi sağlanır.
