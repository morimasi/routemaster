...
import { LeafletNavigationModule } from './modules/navigation-leaflet';
import { GoogleNavigationModule } from './modules/navigation-google';
import { HereNavigationModule } from './modules/navigation-here';
...
function App() {
  ...
  const [driverView, setDriverView] = useState<'stops' | 'leaflet' | 'google' | 'here'>('stops');
  ...
  if (activeRole === 'driver') {
    return (
      <div className="relative">
        <div className="fixed top-2 left-1/2 -translate-x-1/2 z-50 flex gap-1 bg-slate-900/90 backdrop-blur-xl border border-slate-800/60 rounded-xl p-1 text-[10px] font-bold shadow-xl shadow-black/50">
          <button onClick={() => setDriverView('stops')} className={`px-3 py-1.5 rounded-lg transition-all ${driverView === 'stops' ? 'bg-blue-600 text-white shadow-sm shadow-blue-600/30' : 'text-slate-400 hover:text-white'}`}>
            Duraklar
          </button>
          <button onClick={() => setDriverView('leaflet')} className={`px-3 py-1.5 rounded-lg transition-all ${driverView === 'leaflet' ? 'bg-blue-600 text-white shadow-sm shadow-blue-600/30' : 'text-slate-400 hover:text-white'}`}>
            Navigasyon (OSM)
          </button>
          <button onClick={() => setDriverView('google')} className={`px-3 py-1.5 rounded-lg transition-all ${driverView === 'google' ? 'bg-blue-600 text-white shadow-sm shadow-blue-600/30' : 'text-slate-400 hover:text-white'}`}>
            Navigasyon (Google)
          </button>
          <button onClick={() => setDriverView('here')} className={`px-3 py-1.5 rounded-lg transition-all ${driverView === 'here' ? 'bg-blue-600 text-white shadow-sm shadow-blue-600/30' : 'text-slate-400 hover:text-white'}`}>
            Navigasyon (HERE)
          </button>
        </div>
        {driverView === 'leaflet' && <LeafletNavigationModule />}
        {driverView === 'google' && <GoogleNavigationModule />}
        {driverView === 'here' && <HereNavigationModule />}
        {driverView === 'stops' && <DriverHUDModule />}
      </div>
    );
  }
