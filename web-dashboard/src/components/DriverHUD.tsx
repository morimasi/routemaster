...
export const DriverHUD: React.FC = () => {
  ...
  // Live GPS speed flicker simulation
  useEffect(() => {
  ...
  const handlePruneConfirm = useCallback(async () => {
    ...
    setIsPruning(true);
    try {
      await ShuttleXApiService.executeDriverPrune('t-1001', 'r-sabah-1', pendingPruneNode.id, 'd-5001');
  ...
  const handleVoiceCommand = useCallback(async () => {
    ...
    const res = await ShuttleXApiService.parseVoiceIntent(phrase);
    ...
    <div className="min-h-screen bg-black text-white p-4 md:p-6 font-sans flex flex-col gap-5 select-none relative">
      ...
      {!routeComplete && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 flex-1">
          ...
          {/* Left Col 8: Active Stop + Actions */}
          <div className="lg:col-span-8 space-y-5">

            {/* Turn-by-Turn Navigation Band */}
